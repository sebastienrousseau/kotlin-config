...
<p align="center">
  ...
  <a href="https://sebastienrousseau.com/kotlin-config/"><img src="https://img.shields.io/badge/docs-sebastienrousseau.com-brightgreen.svg?style=for-the-badge&logo=github" alt="Documentation" /></a>
...
- [Strict Detekt & Ktlint Quality Standards](#strict-detekt-ktlint-quality-standards) — before and after rule comparison
...
Install `@sebastienrousseau/kotlin-config` as a development dependency:

```bash
# npm
npm install --save-dev @sebastienrousseau/kotlin-config

# pnpm
pnpm add -D @sebastienrousseau/kotlin-config

# yarn
yarn add -D @sebastienrousseau/kotlin-config

# bun
bun add -d @sebastienrousseau/kotlin-config
```

### Build from source

```bash
git clone https://github.com/sebastienrousseau/kotlin-config.git
cd kotlin-config
make            # check + test
```
...
- **Target environment**: Optimized for Kotlin 2.0+ / Detekt.
...
```bash
cp node_modules/@sebastienrousseau/kotlin-config/detekt.yml ./detekt.yml
```

### Or point the tool at the installed file

```bash
detekt --config node_modules/@sebastienrousseau/kotlin-config/detekt.yml --input src
```
...
`@sebastienrousseau/kotlin-config` is a focused satellite of the [`@sebastienrousseau/config`](https://github.com/sebastienrousseau/config) suite — a unified ecosystem of single-purpose, zero-overhead developer configurations designed to share a single design philosophy, strict typing, and zero runtime dependencies.
...
`@sebastienrousseau/kotlin-config` solves this with three deliberate engineering choices:
...
## Strict Detekt & Ktlint Quality Standards

Combines official ktlint formatting with Detekt static code analysis to eliminate code smells and complexity.
...
### After (@sebastienrousseau/kotlin-config Enforced)
...
When maintaining legacy Kotlin codebases targeting obsolete Kotlin 1.3 runtimes.
