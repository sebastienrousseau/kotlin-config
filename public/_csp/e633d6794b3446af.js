...
lp=lm?'/kotlin-config/'+lm[1]:'/kotlin-config';
function load(){if(idx)return Promise.resolve();var sp=lm?'/kotlin-config/'+lm[1]+'/search-index.json':'/kotlin-config/search-index.json';return fetch(sp).then(function(r){return r.json()}).then(function(d){idx=d.entries||[]}).catch(function(){idx=[]})}
